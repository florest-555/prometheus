...
export async function generateNotices({
  ...
  let pkgJson: { name?: string; version?: string; license?: string };
  try {
    pkgJson = JSON.parse(await fs.readFile(path.join(root, 'package.json'), 'utf-8'));
  } catch {
    pkgJson = {};
  }
  const pkg = pkgJson;
  const projectNome = `${pkg.name ?? 'unknown'}@${pkg.version ?? '0.0.0'}`;
