...
async function gerarBaseline(destDir: string, metricas?: Partial<MetricaExecucaoLike>) {
  ...
  const base: Omit<SnapshotPerf, 'hashConteudo'> = {
    ...
    analistasTop: Array.isArray(metricas?.analistas) ? metricas.analistas.slice().sort((a, b) => b.duracaoMs - a.duracaoMs).slice(0, 5).map(a => ({ // prometheus-ignore: inefficient-sort - ordenação necessária para ranqueamento
      nome: a.nome,
