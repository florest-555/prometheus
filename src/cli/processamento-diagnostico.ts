...
// Constantes para timeouts e limites
const DETECTAR_TEMPO_LIMITE_MS_VITEST = 1000;
const DETECTAR_TEMPO_LIMITE_MS_PROD = 30000;
const DETECTAR_TEMPO_LIMITE_MS = process.env.VITEST ? DETECTAR_TEMPO_LIMITE_MS_VITEST : DETECTAR_TEMPO_LIMITE_MS_PROD;
const OCORRENCIAS_LIMITE_CUT = 1000;
const OCORRENCIAS_LIMITE_SLICE = 2000;
...
async function detectarArquetiposComTimeout(ctx: Parameters<typeof detectarArquetipos>[0], baseDir: Parameters<typeof detectarArquetipos>[1], options?: {
  ...
  try {
    const detectPromise = detectarArquetipos(ctx, baseDir, options).catch(e => {
      ...
      try {
        if (config.DEV_MODE && typeof (log as unknown as { erro?: (msg: string) => void }).erro === 'function') {
          const msg = e instanceof Error ? e.message : String(e);
          (log as unknown as { erro: (msg: string) => void }).erro(`Falha detector arquetipos: ${msg}`);
        }
      } catch { }
      return undefined;
    });
    ...
    if (typeof (log as unknown as { debug?: (msg: string) => void }).debug === 'function') {
      (log as unknown as { debug: (msg: string) => void }).debug(`detectarArquetiposComTimeout falhou: ${e instanceof Error ? e.message : String(e)}`);
    }
    return undefined;
  }
}
  ...
  try {
    ...
    if (tiposOcorrencias.size > 0 && !opts.json) {
      ...
      // Resumo básico sempre (não verbose e não executive)
        ...
        if (totalOcorrenciasProcessadas > OCORRENCIAS_LIMITE_CUT) {
    ...
    if (arquetiposResultado) {
      ...
      if (opts.json) {
        ...
        try {
          ...
          // prometheus-ignore: console-in-production - erro ao gerar JSON precisa ser mostrado
      ...
      try {
        ...
        if (config.REPORT_EXPORT_ENABLED) {
          ...
          try {
            ...
            const ocorrenciasLimpas = dedupeOcorrencias(resultadoExecucao.ocorrencias || []).slice(0, OCORRENCIAS_LIMITE_SLICE).map(oc => {
