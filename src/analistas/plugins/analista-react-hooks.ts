...
const MAX_CHARS_HOOK_IN_CONDITIONAL = 160;
...
function collectHookIssues(src: string, relPath: string): Msg[] {
  ...
  const conditionalHooks = [...src.matchAll(new RegExp(`(if|for|while)\\s*\\([^)]*\\)\\s*\\{[\\s\\S]{0,${MAX_CHARS_HOOK_IN_CONDITIONAL}}?use[A-Z][A-Za-z0-9_]*`, 'g'))];
