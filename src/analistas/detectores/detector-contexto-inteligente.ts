...
function analisarDependencias(packageJson?: PackageJson): EvidenciaContexto[] {
  ...
  if (config.VERBOSE) {
    console.log('[SCAN] Analisando dependências:', Object.keys(deps));
  }
  ...
  switch (tecnologia) {
    case 'discord-bot':
      if (!evidencias.some(e => e.valor.includes('intents'))) {
        sugestoes.push('[CONF] Configurar intents específicos no Discord Client para melhor performance');
      }
      if (!evidencias.some(e => e.valor.includes('command'))) {
        sugestoes.push('[DIR] Estruturar commands em pasta dedicada para organização');
      }
      if (!evidencias.some(e => e.valor.includes('rate'))) {
        sugestoes.push('[FAST] Implementar rate limiting para evitar banimento por spam');
      }
      break;
    case 'express-api':
      if (!evidencias.some(e => e.valor.includes('helmet'))) {
        sugestoes.push('[GUARD]️ Adicionar helmet para headers de segurança');
      }
      if (!evidencias.some(e => e.valor.includes('cors'))) {
        sugestoes.push('[ALL] Configurar CORS adequadamente para APIs');
      }
      if (!evidencias.some(e => e.valor.includes('joi') && e.valor.includes('swagger'))) {
        sugestoes.push('[LIST] Implementar validação com Joi e documentação Swagger');
      }
      break;
    case 'cli-modular':
      ...
      if (!evidencias.some(e => e.valor.includes('version'))) {
        sugestoes.push('[PKG] Adicionar comando --version');
      }
      if (!evidencias.some(e => e.valor.includes('config'))) {
        sugestoes.push('[CONF]️ Considerar arquivo de configuração para usuários');
      }
      break;
    case 'electron-app':
      if (!evidencias.some(e => e.valor.includes('preload'))) {
        sugestoes.push('[LOCK] Implementar preload script para comunicação segura');
      }
      if (!evidencias.some(e => e.valor.includes('contextIsolation'))) {
        sugestoes.push('[GUARD]️ Habilitar context isolation para segurança');
      }
      if (!evidencias.some(e => e.valor.includes('updater'))) {
        sugestoes.push('[SYNC] Implementar auto-updater para atualizações automáticas');
      }
      break;
  }
  return sugestoes;
...
export function detectarContextoInteligente(estruturaDetectada: string[], arquivos: FileEntryWithAst[], packageJson?: PackageJson, options?: {
  ...
  if (!options?.quiet && config.VERBOSE) {
    const mensagem = `[SCAN] Package.json completo:${JSON.stringify(packageJson, null, 2)}`;
    ...
    if (packageJson?.dependencies?.vue) {
      const m2 = `[SCAN] Package.json tem Vue: ${packageJson.dependencies.vue}`;
