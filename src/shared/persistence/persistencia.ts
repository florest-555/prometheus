...
async function salvarEstadoImpl<T = unknown>(caminho: string, dados: T): Promise<void> {
  ...
  const tempArquivoCaminho = path.join(dir, `.tmp-bin-${Date.now()}-${crypto.randomUUID().slice(0, 12)}.prometheus`);
...
export async function salvarEstadoAtomico<T = unknown>(caminho: string, dados: T): Promise<void> {
  ...
  const tempArquivoCaminho = path.join(dir, `.tmp-${Date.now()}-${crypto.randomUUID().slice(0, 12)}.json`);
...
export async function salvarBinarioAtomico(caminho: string, dados: Buffer): Promise<void> {
  ...
  const temporaryValor = path.join(dir, `.tmp-${Date.now()}-${crypto.randomUUID().slice(0, 12)}.bin`);
